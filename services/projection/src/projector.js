...
class StateProjector {
  ...
  initCanonicalBaseline(data = {}) {
    // Populate base 30 repos inventory from canonical topology, fall back to seed
    const topoNames = topology.canonicalRepoNames();
    const repos = data.repos && data.repos.length
      ? data.repos
      : (topoNames.length ? topoNames : [
    ...
    for (const r of repos) {
      ...
      const topoEntry = topology.lookup(name);
      const governanceStatus = topoEntry
        ? topology.classifyStatus(name)
        : (name.includes('firetest') ? 'TEMPORARY_FIXTURE' : 'UNREGISTERED');
      const plane = topoEntry ? topology.planeFor(name) : this.inferPlane(name);
      this.repositories.set(name, {
        ...
        governanceStatus,
        plane,
        role: topoEntry ? topoEntry.role : null,
        lifecycle: topoEntry ? topoEntry.lifecycle : null,
      ...
      this.addGraphNode('Repository', name, { name, plane });
    }
    ...
    this.machines.set('lenovo-yoga-local', {
      ...
      status: 'ONLINE',
      cpuUsage: 22, ramUsage: 48, battery: 94,
    ...
    this.missions.set('MISSION-2026-09A', {
      ...
      status: 'EXECUTING', progress: 82, owner: 'JonasAbde',
      budget: { spent: 14.20, cap: 50.00 },
      repos: ['aftergraph', 'studio'],
      evidenceCount: 14, verificationStatus: 'PARTIAL'
    });
    this.missions.set('MISSION-2026-09B', {
      ...
      status: 'COMPLETED', progress: 100, owner: 'sentinel-bot',
      budget: { spent: 4.50, cap: 25.00 },
      repos: ['sentinel', 'sentinel-bench'],
      evidenceCount: 26, verificationStatus: 'VERIFIED'
    });
    this.missions.set('MISSION-2026-09C', {
      ...
      status: 'IN_REVIEW', progress: 64, owner: 'forge',
      budget: { spent: 8.90, cap: 30.00 },
      repos: ['trust-gateway', 'governance'],
      evidenceCount: 9, verificationStatus: 'PARTIAL'
    });
  }
